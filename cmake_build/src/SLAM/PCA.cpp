...
public:
    void Get_mean(cv::Mat pts){
        cv::reduce(pts, mean_, 0, cv::REDUCE_AVG);
        std::cout << "Mean_ = " << mean_.at<double>(0) << ", " << mean_.at<double>(1) << std::endl;
    }
    void Get_covariacne(cv::Mat pts){
        cv::Mat centered = pts - cv::repeat(mean_, pts.rows, 1);
        cov_ = (centered.t() * centered) / double(pts.rows - 1);
    ...
    void Get_normalVector(void){
        ...
        // cv::eigen(cov_, eigenvalues, eigenvectors);
        // std::cout << "eigen_vector = " << eigenvectors << std::endl;
    }
    ...
    double calc_orthogonal_distance(cv::Mat pts){
        ...
        for(int row=0; row<pts.rows; row++){
            pts.at<double>(row,0) -= mean_.at<double>(0);
            pts.at<double>(row,1) -= mean_.at<double>(1);
        }

        cv::Mat orthogonal_distance = cv::Mat(pts.rows, 1, CV_64F);
        for(int row=0; row<pts.rows; row++){
            cv::Point2d dummy = cv::Point2d(pts.at<double>(row,0), pts.at<double>(row,1));
            orthogonal_distance.at<double>(row) = fabs((dummy.x * normal_vector.x) + (dummy.y * normal_vector.y));
        }
        double mean = cv::mean(orthogonal_distance)[0];
        double variance = 0;
        for (int i = 0; i < orthogonal_distance.rows; ++i) {
            double diff = orthogonal_distance.at<double>(i, 0) - mean;
            variance += diff * diff;
        }
        variance /= orthogonal_distance.rows;
        std::cout << "variance: " << variance << std::endl;
        std::cout << "Standard_deviation: " << sqrt(variance) << std::endl;

        // double orthogonal_mean_distance;
        // for(int row=0; row<orthogonal_distance.rows; row++){orthogonal_mean_distance += orthogonal_distance.at<double>(row);}
    ...
    void clustering_conditional_1(double query_distance, double cluster_distance_variance){
        ...
        // double threshold_orthogonal_distance = 0.1;
...
{
    cv::Point2d Query_data = cv::Point(109, 301);    //199,300,,,,limit 208,300 
